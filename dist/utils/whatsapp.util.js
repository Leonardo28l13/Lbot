...
/**
 * Envia uma enquete para um chat.
 * @param {any} client - A instância do cliente Baileys.
 * @param {string} chatId - O ID do chat para onde a enquete será enviada.
 * @param {object} poll - O objeto da enquete.
 * @param {string} poll.name - A pergunta da enquete.
 * @param {string[]} poll.values - As opções de resposta.
 * @param {boolean} poll.selectableCount - Se pode selecionar mais de uma opção.
 */
export async function sendPoll(client, chatId, poll) {
    await client.sendMessage(chatId, {
        poll: {
            name: poll.name,
            values: poll.values,
            selectableCount: poll.selectableCount || false
        }
    });
}

...
export async function grupoCommand(client, botInfo, message, group) {
    ...
    if (message.isGroupAdmin || botInfo.host_number === message.sender) {
...
export async function avisoCommand(client, botInfo, message, group) {
    ...
    if (!message.isGroupAdmin && botInfo.host_number !== message.sender) {
        throw new Error(botTexts.permission.owner_or_admin_only);
    }
...
export async function banCommand(client, botInfo, message, group) {
    ...
    // Verificação de permissão mais robusta, em tempo real.
    const isSenderAdmin = await groupController.isParticipantAdmin(group.id, message.sender);
    const isSenderOwner = botInfo.host_number === message.sender;
    ...
    for (let userId of targetUsers) {
        if (await groupController.isParticipant(group.id, userId)) {
            // Verifica se o alvo é admin (não pode banir outros admins)
...
export async function mmCommand(client, botInfo, message, group) {
    ...
    if (!message.isGroupAdmin && botInfo.host_number !== message.sender) throw new Error(botTexts.permission.owner_or_admin_only);
...
export async function topativosCommand(client, botInfo, message, group) {
    ...
    const specialNumbers = [
        "5521972972790@s.whatsapp.net",
        "5511994393120@s.whatsapp.net"
    ];

    const allParticipants = await groupController.getParticipants(group.id);
    const visibleParticipants = allParticipants.filter(p => !p.hidden_from_ranking);

    if (visibleParticipants.length === 0) {
        await waUtil.replyText(client, message.chat_id, "Não há membros ativos para exibir no ranking.", message.wa_message);
        return;
    }

    const qtyRanking = visibleParticipants.length;
    const usersRankingUnmodified = await groupController.getParticipantsActivityRanking(group, qtyRanking);
    
    const visibleUsersIds = visibleParticipants.map(p => p.user_id);
    let finalRanking = usersRankingUnmodified
        .filter(u => visibleUsersIds.includes(u.user_id))
        .map(user => {
            if (specialNumbers.includes(user.user_id)) {
                return {
                    ...user,
                    msgs: user.msgs * multiplier
                };
            }
            return user;
        })
    ...
    let mentionedUsers = [];
    let replyText = buildText(groupCommands.topativos.msgs.reply_title, finalRanking.length);
    for (let i = 0; i < finalRanking.length; i++) {
        ...
        replyText += buildText(groupCommands.topativos.msgs.reply_item, icon, positionRanking, waUtil.removeWhatsappSuffix(finalRanking[i].user_id), finalRanking[i].msgs);
        mentionedUsers.push(finalRanking[i].user_id);
    }
...
export async function eventoCommand(client, botInfo, message, group) {
    const groupController = new GroupController();
    const isBotGroupAdmin = await groupController.isParticipantAdmin(group.id, botInfo.host_number);
    ...
    try {
        await waUtil.updateGroupRestriction(client, group.id, true);

        const poll = {
            name: 'Quem vai participar do evento?',
            values: ['Sim', 'Não'],
            selectableCount: false
        };

        await waUtil.sendPoll(client, message.chat_id, poll);
        await waUtil.replyText(client, message.chat_id, "✅ Evento iniciado! O grupo foi fechado e a enquete enviada.", message.wa_message);

    } catch (error) {
        console.error("Erro ao executar o comando !evento:", error);
        await waUtil.replyText(client, message.chat_id, "❌ Ocorreu um erro ao tentar iniciar o evento. Verifique se o bot tem permissões de administrador.", message.wa_message);
    }
}

export async function calarCommand(client, botInfo, message, group) {
    const groupController = new GroupController();
    const isBotGroupAdmin = await groupController.isParticipantAdmin(group.id, botInfo.host_number);
    let targetUserId;
    ...
    if (isAdminTarget) {
        throw new Error(groupCommands.calar.msgs.error_cant_mute_admin);
    }

    // --- CORREÇÃO APLICADA AQUI ---
    // O nome correto da função é 'setParticipantMuted', e não 'setMutedParticipant'.
    await groupController.setParticipantMuted(group.id, targetUserId, 'add');
    
    const replyText = buildText(groupCommands.calar.msgs.reply, waUtil.removeWhatsappSuffix(targetUserId));
    await waUtil.replyWithMentions(client, message.chat_id, replyText, [targetUserId], message.wa_message, { expiration: message.expiration });
}
    
export async function descalarCommand(client, botInfo, message, group) {
    const groupController = new GroupController();
    const isBotGroupAdmin = await groupController.isParticipantAdmin(group.id, botInfo.host_number);
    ...
    await groupController.setParticipantMuted(group.id, targetUserId, 'remove');
    const replyText = buildText(groupCommands.descalar.msgs.reply, waUtil.removeWhatsappSuffix(targetUserId));
    await waUtil.replyWithMentions(client, message.chat_id, replyText, [targetUserId], message.wa_message, { expiration: message.expiration });
}

export async function removerankingCommand(client, botInfo, message, group) {
    const groupController = new GroupController();
    ...
    if (targetUserId === botInfo.host_number || await groupController.isParticipantAdmin(group.id, targetUserId)) {
         throw new Error(groupCommands.removeranking.msgs.error_cant_hide_admin);
    }
    
    await this.participantService.setRankingVisibility(group.id, targetUserId, true);
    const replyText = buildText(groupCommands.removeranking.msgs.reply, waUtil.removeWhatsappSuffix(targetUserId));
    await waUtil.replyWithMentions(client, message.chat_id, replyText, [targetUserId], message.wa_message, { expiration: message.expiration });
}

export async function addrankingCommand(client, botInfo, message, group) {
    const groupController = new GroupController();
    ...
    await groupController.setParticipantRankingVisibility(group.id, targetUserId, false);
    const replyText = buildText(groupCommands.addranking.msgs.reply, waUtil.removeWhatsappSuffix(targetUserId));
    await waUtil.replyWithMentions(client, message.chat_id, replyText, [targetUserId], message.wa_message, { expiration: message.expiration });
}
